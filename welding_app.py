...
c = conn.cursor()
c.executescript('''
CREATE TABLE IF NOT EXISTS areas (area_id INTEGER PRIMARY KEY AUTOINCREMENT, area_name TEXT UNIQUE);
CREATE TABLE IF NOT EXISTS joints (
id INTEGER PRIMARY KEY AUTOINCREMENT,
area_name TEXT, line_no TEXT, tag_no TEXT, drawing_no TEXT,
joint_dia REAL, wps_no TEXT, welder_id TEXT, status TEXT DEFAULT 'Open'
);
CREATE TABLE IF NOT EXISTS welders (welder_id TEXT PRIMARY KEY, name TEXT);
''')
conn.commit()
return conn
...
st.write("Headers needed: line_no, tag_no, drawing_no, joint_dia, wps_no")
...
st.download_button(label="Download Excel Report", data=buffer, file_name="WeldReport.xlsx")
else:
st.info("No data yet.")
